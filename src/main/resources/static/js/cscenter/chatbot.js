  		...
  		navigator.mediaDevices.getUserMedia(constraints)
  			.then(stream => {
  				...
  				recordBtn.onclick = () => {
  					...
  					recordBtn.style.background = "#abb3a7"; // [녹음] 버튼 배경색
 			...
 			success:function(result){
				...
				for(var b in bubbles){
					if(bubbles[b].type == 'text'){ // 기본 답변인 경우
						/* chatBox에 받은 메시지 추가 */
							$('#chatBox').append('<div class="msgBox receive"><span id="in"><span>Mu</span><br><span>' + 
						...
						if(bubbles[b].data.cover.type=="image"){//이미지 이면
							...
							if(bubbles[b].data.contentTable == null){
								...
								$("#chatBox").append("<div class='msgBox receive'><span id='in'><span>Mu</span><br><span>" + bubbles[b].data.cover.data.description+ "</span></span></div><br><br>");	
								// 챗봇으로 부터 받은 텍스트 답변을 음성으로 변환하기 위해 TTS 호출									   
